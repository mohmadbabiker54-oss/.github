  ...
  const generateSecurePDF = async () => {
    ...
    try {
      ...
      const canvas = await html2canvas(pdfContentRef.current, {
        ...
        windowWidth: 800,
        onclone: (clonedDoc) => {
          const invoiceEl = clonedDoc.getElementById('pdf-content-to-capture');
          const receiptEl = clonedDoc.getElementById('receipt-content-to-capture');
          if (invoiceEl) {
      ...
      if (receipt && receiptContentRef.current) {
        const receiptCanvas = await html2canvas(receiptContentRef.current, {
          scale: 2,
          useCORS: true,
          backgroundColor: '#ffffff',
          windowWidth: 800,
        ...
        const receiptImgData = receiptCanvas.toDataURL('image/jpeg', 0.95);
        const receiptPdfHeight = (receiptCanvas.height * pdfWidth) / receiptCanvas.width;
        
        // Add a new page with custom height to fit the full receipt
        // A4 width is 210mm
        pdf.addPage([210, receiptPdfHeight]);
        pdf.addImage(receiptImgData, 'JPEG', 0, 0, 210, receiptPdfHeight, undefined, 'FAST');
      }
      ...
      >
        ...
        <div className="fixed left-[-9999px] top-0 pointer-events-none z-[-100]">
          ...
          <div id="receipt-content-to-capture" ref={receiptContentRef} style={{ width: '800px', backgroundColor: '#ffffff', padding: '40px', fontFamily: 'Cairo, sans-serif' }} dir="rtl">
            <div style={{ borderBottom: '4px solid #1f2937', paddingBottom: '24px', marginBottom: '32px', textAlign: 'center' }}>
              <h2 style={{ fontSize: '32px', fontWeight: '900', color: '#4b5563', textTransform: 'uppercase', letterSpacing: '0.1em' }}>إشعار الدفع المرفق (زون)</h2>
            </div>
